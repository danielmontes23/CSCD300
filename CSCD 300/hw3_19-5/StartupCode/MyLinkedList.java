public class MyLinkedList {
	...
	private MyLinkedList reverse(ListNode node) {
		if (node == null) {
			return new MyLinkedList(); // Calls constructor and an empty list
		}

		MyLinkedList subList = reverse(node.next); // Will make the subList print in reverse minus the last node
		subList.addLast(node.data); // Helper method to add node A to the end of the list
		return subList; // change this line of code as needed
	}
	...
	private ListNode reverse(ListNode first, ListNode second) {
		if(second == null){
			return first; // If second is null return the first
		}

		ListNode newHead = reverse(second, second.next); //Add node A after node B
		second.next = first; // Adding node A
		first.next = null; // Making nodeA.next null
